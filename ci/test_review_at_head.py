"""The `Review at head` gate — scripts/review_at_head.py. Run: python3 ci/test_review_at_head.py

This script is the sole required context on `main`, so these are tests of a GUARD: each
case is a way the gate used to pass a head it should not have (protoLabsAI/qaEngineer#73,
ported from protoLabsAI/protoAgent#3543), plus the incident it exists to catch — a verdict for an older
head must not satisfy the merged one.

Stdlib unittest, like test_protopatch_gate.py: CI here has no pytest. protoAgent's
